...
class NokAugAL5Dataset(Dataset):
    def __init__(
            self,
        ...
        self.samples_df = pd.read_csv(f"{root}/nokdb-samples-al5{self.split}.csv")
...
class NokAugAL5DataModule(LightningDataModule):
    ...
    def prepare_data(self):
        ...
        NokAugAL5Dataset(root=self.data_dir)

    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.data_train         = NokAugAL5Dataset(root=self.data_dir, split="train",      inflate=self.inflate)
            self.data_validation    = NokAugAL5Dataset(root=self.data_dir, split="validation", inflate=self.inflate)
        elif stage == 'test':
            self.data_test          = NokAugAL5Dataset(root=self.data_dir, split="test",       inflate=self.inflate)
