...
class M11_x18(LightningModule):
    ...
    def _step(self, batch, batch_idx, log_results=0, label=None):
        father, mother, child, child_gender, f_image, m_image, c_image, loss_weight = batch

        child_hat = self.forward(father, mother)

        child_hat_mse = torch.mean(torch.mean(F.mse_loss(child_hat, child, reduction="none").flatten(1), dim=1) / loss_weight)
        loss = child_hat_mse
...
def main():
    ...
    utils.core.train(M11_x18, config=config, early_stopping_delta=0.0, max_epochs=100, test=True)
