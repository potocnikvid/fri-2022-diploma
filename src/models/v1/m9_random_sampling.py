...
class M9_RandomSampling(LightningModule):
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        self.encoder = nn.Sequential(
            ...
            # nn.ReLU(),
            # nn.Linear(self.ae.latent_dim, self.ae.latent_dim),
        )
    ...
    def encode(self, input, gender):
        input = input.view(-1, 18*512)
        gender = gender.view(-1, 1)

        input_zip = self.ae.encode(input)
        # input_enc = self.encoder(torch.cat([input_zip, gender], dim=1))
        input_enc = self.encoder(input_zip)

        return input_enc

    def random_sample(self, father, mother):
    ...
    def decode(self, child_enc, gender):
        gender = gender.view(-1, 1)
        # child_enc = self.decoder(torch.cat([child_enc, gender], dim=1))
        child_zip = self.decoder(child_enc)
    ...
    def forward(self, father, mother, gender):
        ...
        f_enc = self.encode(father, torch.ones(batch_size, device=self.device))
        m_enc = self.encode(mother, torch.zeros(batch_size, device=self.device))
        c_enc = self.random_sample(f_enc, m_enc)
        return self.decode(c_enc, gender)
    ...
    def _construct_image_grid(self, batch, n=4):
        ...
        child_hat_0 = self.forward(father, mother, child_gender)

        pprint(father.shape)
        pprint(child_hat_0.shape)

        child_hat_1 = self.forward(father, mother, child_gender)
        child_hat_gender_inverse_0 = self.forward(father, mother, child_gender_inverse)
        child_hat_gender_inverse_1 = self.forward(father, mother, child_gender_inverse)
...
def main():
    ...
    utils.core.train(M9_RandomSampling(**config), batch_size=64, early_stopping_delta=0.0, max_epochs=1)
