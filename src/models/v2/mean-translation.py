...
class MeanTranslation(LightningModule):

    def __init__(self, **kwargs):
        super().__init__()  
        self.save_hyperparameters()

        self.translation = nn.Parameter(torch.zeros((18, 512), device=self.device))
    ...
    def forward(self, father, mother):
        ...
        return torch.mean(torch.stack([father, mother], dim=0), dim=0) + self.translation
...
def main():
    ...
    with wandb.init(name=model_name, config=config, save_code=True):
        ...
        model = MeanTranslation.load_from_checkpoint(trainer.checkpoint_callback.best_model_path)
