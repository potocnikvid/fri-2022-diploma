...
class BaseEvaluator():
    ...
    def get_embedding(self, image, model=None, model_name=None):
    ...
    def get_embeddings(self, images, model):
        embeddings = []
        for image in images:
            embeddings.append(self.get_embedding(image, model))
    ...
    def evaluate(self, image, image_hat, model_name):
        ...
        image_embedding = self.get_embedding(image, model)
        image_hat_embedding = self.get_embedding(image_hat, model)
        res = self.compare_embeddings(image_embedding[0], image_hat_embedding[0])
        return res

    def evaluate_batch(self, images, images_hat, model_name):
        model = DeepFace.build_model(model_name)
        image_embeddings = self.get_embeddings(images, model)
        image_hat_embeddings = self.get_embeddings(images_hat, model)
        res = []
